...
load("analysisData.rda")
#names(analysisData)
analysisData <- subset(analysisData, agea > 14 & agea < 36)
...
analysisData[,response] <- apply(analysisData[,response],2,scale)
output <- list()
for( i in seq_along(response)){
	cat("\nNow working on ", response[i], "\n")
	form <- as.formula(paste(response[i]," ~ GR*CNTeffect + (1|cntry/essround)"))
...
for( i in seq_along(response)){
	cat("\nNow working on ", response[i], "\n")
	form <- as.formula(paste(response[i]," ~ GR*CNTeffect*ISEI + (1|cntry/essround)"))
	output[[i]] <- lmer(form, data = analysisData, weights = dweight)
} 
